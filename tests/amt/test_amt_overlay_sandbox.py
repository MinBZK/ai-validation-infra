...
def manifest() -> list[dict]:
    result = subprocess.run(["kubectl", "kustomize", "apps/amt/overlays/sandbox"], capture_output=True)  # noqa: S607
...
def test_ingress(manifest):
    ...
    assert ingresses[0]["spec"]["rules"][0]["host"] == "amt.stag.apps.digilab.network"
    assert ingresses[0]["spec"]["rules"][0]["http"]["paths"][0]["pathType"] == "Prefix"
    assert ingresses[1]["spec"]["rules"][0]["host"] == "pgadmin.stag.apps.digilab.network"
